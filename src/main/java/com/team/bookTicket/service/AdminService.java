...
public class AdminService {
	...
	/*------------------------影片管理-------------*/
	public void addMovie(Course course) {
	...
	public List<Course> allMovie(Integer page, Integer limit){
		if(page!=null && limit!=null) {
			return courseDao.all((page-1)*limit,limit);
		}else {
			return courseDao.all(null, null);
		}
	}
	public Course getMovie(Integer id) {
		return courseDao.findById(id);
	}
	public void updateMovie(Course course) {
	...
	/**
	 * 为房间中的所有座位生成一张影票
	 * @param ticket 含有room、course、price和playtime；其中row和column表示行列数，不是位置
	 */
